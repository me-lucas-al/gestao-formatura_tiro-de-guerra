...
type FamilyMemberFiltersProps = {
  filters: { name: string; status: string };
  totalCount: number;
};

export function FamilyMemberFilters({
  filters,
  totalCount,
}: FamilyMemberFiltersProps) {
  ...
  useEffect(() => {
    updateParams({ familiar_name: debouncedName });
    ...
    <div className="p-4 border-b border-slate-200 bg-slate-50">
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <Select
          ...
          onValueChange={(value) => updateParams({ familiar_status: value })}
        >
      ...
      <p className="text-xs text-slate-500 mt-2 uppercase tracking-wide">
        {totalCount} familiar(es) encontrado(s)
      </p>
    </div>
  );
}
